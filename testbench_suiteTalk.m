...
% validateAudioPlugin.
%
% Automatically generated by validateAudioPlugin 08-Jun-2025 13:16:03 UTC-04:00
...
plugin.ToggleEnhance = 'Off';
plugin.ToggleEnhance = 'On';
plugin.ToggleComb = 'On';
plugin.ToggleComb = 'Off';
plugin.ToggleLimiter = 'Off';
plugin.ToggleLimiter = 'On';
...
for sampleRate = sampleRates
    ...
    for i = 1:numel(frameSizes)
        ...
        val = fromNormalizedEnhanceLevel(mod(floor((i-1)./27),3)/2);
        plugin.EnhanceLevel = val;
        paramState.EnhanceLevel = val;
        checkForTampering(plugin, paramState, sampleRate, ...
            'Setting parameter ''EnhanceLevel''');

        val = fromNormalizedToggleEnhance(mod(floor((i-1)./81),3)/2);
        plugin.ToggleEnhance = val;
        paramState.ToggleEnhance = val;
        checkForTampering(plugin, paramState, sampleRate, ...
            'Setting parameter ''ToggleEnhance''');

        val = fromNormalizedcombAmount(mod(floor((i-1)./243),3)/2);
        plugin.combAmount = val;
        paramState.combAmount = val;
        checkForTampering(plugin, paramState, sampleRate, ...
            'Setting parameter ''combAmount''');

        val = fromNormalizedToggleComb(mod(floor((i-1)./729),3)/2);
        plugin.ToggleComb = val;
        paramState.ToggleComb = val;
        checkForTampering(plugin, paramState, sampleRate, ...
            'Setting parameter ''ToggleComb''');

        val = fromNormalizedRatio(mod(floor((i-1)./2187),3)/2);
        plugin.Ratio = val;
        paramState.Ratio = val;
        checkForTampering(plugin, paramState, sampleRate, ...
            'Setting parameter ''Ratio''');

        val = fromNormalizedToggleLimiter(mod(floor((i-1)./6561),3)/2);
        plugin.ToggleLimiter = val;
        paramState.ToggleLimiter = val;
        checkForTampering(plugin, paramState, sampleRate, ...
            'Setting parameter ''ToggleLimiter''');
...
if ~isequal(paramState.EnhanceLevel, plugin.EnhanceLevel)
    error('ValidateAudioPlugin:ParamChanged', ...
        '%s changed parameter ''EnhanceLevel'' from %g to %g.', ...
        cause, paramState.EnhanceLevel, plugin.EnhanceLevel);
end
if ~isequal(paramState.ToggleEnhance, plugin.ToggleEnhance)
    error('ValidateAudioPlugin:ParamChanged', ...
        '%s changed parameter ''ToggleEnhance'' from ''%s'' to ''%s'.'', ...
        cause, paramState.ToggleEnhance, plugin.ToggleEnhance);
end
...
if ~isequal(paramState.ToggleComb, plugin.ToggleComb)
    error('ValidateAudioPlugin:ParamChanged', ...
        '%s changed parameter ''ToggleComb'' from ''%s'' to ''%s'.'', ...
        cause, paramState.ToggleComb, plugin.ToggleComb);
end
...
if ~isequal(paramState.ToggleLimiter, plugin.ToggleLimiter)
    error('ValidateAudioPlugin:ParamChanged', ...
        '%s changed parameter ''ToggleLimiter'' from ''%s'' to ''%s'.'', ...
        cause, paramState.ToggleLimiter, plugin.ToggleLimiter);
end
...
paramState.EnhanceLevel = plugin.EnhanceLevel;
paramState.ToggleEnhance = plugin.ToggleEnhance;
paramState.combAmount = plugin.combAmount;
paramState.ToggleComb = plugin.ToggleComb;
paramState.Ratio = plugin.Ratio;
paramState.ToggleLimiter = plugin.ToggleLimiter;
end
...
function val = fromNormalizedEnhanceLevel(normval)
val = -12 + (0--12)*normval;
end

function val = fromNormalizedToggleEnhance(normval)
...
function val = fromNormalizedToggleComb(normval)
idx = floor( 0.5 + normval * 1);
switch idx
    case 0
        val = 'On';
...
function val = fromNormalizedToggleLimiter(normval)
idx = floor( 0.5 + normval * 1);
switch idx
    case 0
        val = 'On';
